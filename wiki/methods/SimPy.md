...
SimPy는 [[methods/Discrete-Event Simulation|이산사건 시뮬레이션]]의 개념을 작은 실험 모델로 검증하는 데 유용하다. 이후 모델을 디지털트윈이나 [[methods/Simulation-based Optimization|시뮬레이션 기반 최적화]]에 연결하려면, 코드 실행 자체보다 상태 동기화, 입력 불확실성, 검증·타당화, 계산시간을 추가로 설계해야 한다.
