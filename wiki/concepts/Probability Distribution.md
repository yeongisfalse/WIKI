...
[[methods/Discrete-Event Simulation|이산사건 시뮬레이션]]의 도착 간격·처리시간·고장시간은 확률분포로 모델링되는 경우가 많다. [[concepts/Poisson Process|포아송 과정]]은 사건 도착의 한 모델이고, [[concepts/Queueing Theory|대기행렬 이론]]은 그 입력이 시스템 성능에 미치는 영향을 분석한다. 최적화에서는 평균 조건의 분포뿐 아니라 변동·꼬리·시나리오별 결과를 목적함수와 제약에 반영할 수 있다.
