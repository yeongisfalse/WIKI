...
- 포아송 과정은 시간 구간의 사건 수를 모델링하며, 균질한 경우 단위시간당 평균 도착률 `λ` 하나로 기본 분포와 도착간격을 결정한다.
- 독립 증분과 stationary increment가 핵심 가정이다. 실제 도착이 군집화되거나 시간에 따라 변하거나 서로 영향을 주면 기본 포아송 모형이 맞지 않을 수 있다.
- 이산사건 시뮬레이션에서는 도착 이벤트를 생성하는 입력 모델로 사용되지만, `λ` 추정과 비균일성 검증이 모델 사용보다 먼저 이루어져야 한다.
