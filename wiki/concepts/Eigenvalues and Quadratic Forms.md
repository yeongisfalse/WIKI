...
- 고유값·고유벡터는 선형변환이 특정 방향을 얼마나 늘리거나 줄이는지 나타내고, 대각화는 그 방향을 좌표축처럼 사용해 변환을 단순화한다.
- 이차형식 `xᵀQx`의 부호성은 곡면의 모양과 최적화 문제의 국소 곡률을 결정한다. 대칭 행렬의 고유값 부호가 positive/negative/indefinite 구조를 판별한다.
- 대각화 가능성, 대칭성, 수치 안정성은 서로 다른 조건이다. 이론적 분해가 가능하더라도 실제 계산에서는 조건수와 근사 오차를 확인해야 한다.
