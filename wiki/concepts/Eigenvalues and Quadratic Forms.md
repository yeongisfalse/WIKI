...
두 번 미분 가능한 목적함수의 헤시안(Hessian) `H`는 국소 곡률을 표현한다. 제약이 없는 내부점에서 `H`의 부호성과 gradient를 함께 보면 local minimum·maximum·saddle point를 판별하는 기준을 만들 수 있다. 볼록 최적화에서 양의 준정부호 Hessian은 중요한 구조적 조건이지만, 제약·경계·수치오차를 별도로 검토해야 한다.
...
- Positive Definite가 곧 모든 최적화 문제의 유일한 전역 최적해를 보장하는 것은 아니다. 목적함수·정의역·제약조건을 함께 확인한다.
