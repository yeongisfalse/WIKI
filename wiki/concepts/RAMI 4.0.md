...
- RAMI 4.0은 산업 시스템을 계층(hierarchy), 수명주기·가치 흐름(life cycle/value stream), 기능 계층(layers)의 세 축으로 배치하는 참조 모델이다.
- 각 축은 서로 다른 질문에 답한다. 무엇을 연결하는지, 언제의 제품 정보를 다루는지, 어떤 기능·정보 처리가 필요한지를 분리해서 볼 수 있다.
- RAMI 4.0은 구현 설계도나 통신 프로토콜 그 자체가 아니다. 실제 시스템에서는 각 셀의 데이터·책임·인터페이스·검증 증거를 별도로 정의해야 한다.
