...
- 대기행렬 이론은 도착률·서비스율·서버 수·서비스 규칙·대기공간이 대기시간, WIP, 처리량, 이용률에 미치는 영향을 분석한다.
- 평균 도착률이 평균 서비스율보다 낮아도 변동성과 자원 공유 때문에 대기가 커질 수 있다. 안정성 조건과 변동성 가정을 함께 확인해야 한다.
- Little's Law는 장기 평균 관계를 제공하지만, 원인·분포·병목 위치를 알려주지는 않는다. 제조 의사결정에는 DES나 상세한 대기모형이 추가로 필요할 수 있다.
