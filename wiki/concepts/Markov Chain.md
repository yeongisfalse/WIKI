...
- **접근 가능성(Accessibility)**: 어떤 유한 단계 안에 `i`에서 `j`로 갈 확률이 양수인 경우
- **통신(Communication)**: 서로 양방향으로 접근 가능한 경우
- **기약(Irreducible)**: 모든 상태가 하나의 통신 클래스에 속하는 경우
- **주기성(Periodicity)**: 상태로 되돌아오는 가능한 단계들의 최대공약수로 정의되는 성질
- **흡수상태(Absorbing State)**: 한 번 들어가면 계속 그 상태에 머무는 상태
- **재귀·일시 상태(Recurrent·Transient)**: 장기적으로 자기 상태로 돌아올 가능성에 따른 분류

상태 분류는 단순한 용어 목록이 아니라, 장기 안정상태·고장 흡수·복구 가능성·초기상태의 영향이 어떻게 달라지는지를 판단하는 도구다.

## 혜영님의 연구와의 관련성

마르코프 체인은 이산사건 시뮬레이션의 확률적 상태 입력이나 설비 고장·복구·품질상태 전이 모델의 후보가 될 수 있다. 다만 실제 시스템의 미래가 현재 상태만으로 충분히 설명되는지, 상태를 세분화해야 하는지, 전이확률이 시간에 따라 변하는지를 데이터와 도메인 지식으로 확인해야 한다. 전이 모델을 [[methods/Simulation-based Optimization|시뮬레이션 기반 최적화]]에 넣을 때는 전이확률의 불확실성과 정책 변화도 함께 기록해야 한다.
