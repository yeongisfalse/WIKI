...
- 벡터공간은 덧셈과 스칼라배에 대해 닫힌 대상들의 구조이며, span·선형독립·기저·차원은 그 공간을 표현하는 최소 좌표 구조를 설명한다.
- 선형변환은 덧셈과 스칼라배를 보존하는 함수이고, 기저를 선택하면 행렬로 표현할 수 있다. 같은 변환도 기저에 따라 행렬 표현이 달라진다.
- null space와 range는 변환으로 사라지는 방향과 도달 가능한 방향을 분리하며, 선형시스템·최적화·데이터 차원 축소의 공통 언어가 된다.
...
비동차 선형시스템 `Ax=b`가 일관되고 특수해 `x_p`를 가진다면 해집합은

$$
x_p+Nul(A)
$$

로 표현되는 아핀 집합(affine set)이다. `b=0`인 동차 시스템의 해집합은 부분공간이지만, 일반적인 `Ax=b`의 해집합은 원점을 포함하지 않으므로 부분공간이 아닐 수 있다.

최적화의 feasible region도 일반적으로 벡터 부분공간이 아니다. 예를 들어 `x≥0`, `Ax≤b`, `Cx=d`, 변수의 상·하한으로 정의되는 집합은 보통 볼록집합(convex set), 다면체(polyhedron) 또는 폴리토프(polytope)로 분류한다. 이런 집합은 원점을 포함하지 않거나 덧셈·스칼라 곱에 닫혀 있지 않을 수 있다. 선형계획법의 simplex method는 feasible region이 벡터공간이기 때문이 아니라, 선형 제약이 만드는 다면체 구조를 이용한다.
...
특히 `L:\mathbb{R}^n\to\mathbb{R}^m`에서 `n>m`이면 `L`은 one-to-one일 수 없고, `m>n`이면 `L`은 onto일 수 없다. 이는 각각 입력 차원이 출력 차원보다 큰 경우의 필연적인 kernel과, 출력공간 전체를 채울 수 없는 range에서 비롯된다.
...
- `Ax=b`의 해집합과 최적화의 feasible region은 모두 부분공간이라고 가정하면 안 된다. 동차식의 해집합은 부분공간이고, 비동차식의 해집합은 일반적으로 affine set이다.
