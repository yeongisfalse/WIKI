...
- 선형시스템 `Ax=b`의 핵심 질문은 해의 존재 여부, 해의 개수, 해집합의 구조이며, 이 세 가지는 rank와 null space로 연결된다.
- 행 연산은 해집합을 보존하면서 시스템을 REF/RREF로 바꾸는 계산 절차다. `rank(A)`와 `rank([A|b])`를 비교하면 모순 여부를 판별할 수 있다.
- 최적화와 데이터 모델에서는 독립적인 제약의 수, 식별 가능한 방향, 자유도, 중복 변수, 최소제곱 해의 존재가 같은 선형대수 구조로 나타난다.
