...
- 확률 부등식은 정확한 분포를 몰라도 평균·분산·비음성 같은 제한된 정보로 사건 확률의 상한 또는 하한을 제시한다.
- Markov 부등식은 비음수 변수와 평균을, Chebyshev 부등식은 평균과 분산을 사용한다. 정보가 적은 만큼 보장은 넓지만 보통 느슨하다.
- 부등식은 분포 가정을 숨기지 않는 보수적 기준으로 유용하지만, 상한을 실제 확률이나 정확한 예측값으로 해석해서는 안 된다.
