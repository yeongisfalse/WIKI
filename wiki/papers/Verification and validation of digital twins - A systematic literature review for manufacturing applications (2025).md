...
- 이 systematic literature review는 제조 디지털트윈 157편에서 구현 검증(verification)과 사용 맥락 타당화(validation)가 어떻게 보고되는지 분석한다.
- 핵심 구분은 “모델을 요구사항대로 만들었는가”와 “특정 목적에서 현실을 충분히 대표하는가”이며, 둘을 모두 수행한 연구가 많지 않다는 점이다.
- 4R capability와 V&V 기법의 사용 현황은 평가 프레임으로 유용하지만, 문헌 검색 범위와 저자 분류 기준에 의존하므로 개별 모델의 신뢰성을 대신 증명하지 않는다.
...
이 리뷰가 제공하는 가장 중요한 읽기 틀은 검증·타당화를 하나의 “정확도 수치”로 축약하지 않는 것이다. 요구사항 추적성, 코드·논리 점검, 관측자료 비교, 전문가 검토, 민감도·불확실성 분석은 서로 다른 질문에 답하므로, 어떤 증거가 어떤 주장에 대응하는지 기록해야 한다.

또한 디지털트윈 capability가 높아질수록 실시간 동기화와 의사결정 영향이 커지므로 V&V의 대상과 빈도도 달라질 수 있다. 모델 변경, 데이터 분포 변화, 운영 목적 변경이 발생하면 과거의 검증 결과를 그대로 재사용할 수 있는지 재평가해야 한다.
