...
- 명시적 승인에 따라 proposals.md의 pending 제안 P-001–P-008을 모두 승인하고 반영했다.
...
- P-004의 통합 최적화 루프는 확정된 사실이 아니라 해당 연구 맥락에 맞춘 medium 신뢰도의 연구 아이디어 후보로 표시했다.
...
- 개념·방법론 초안에서 반복적인 개인화 관련성 섹션을 제거했다.
...
## [2026-08-02] ingest | 확률·통계·확률과정 확장

- 기존 확률·통계 노트 9개와 확률과정 응용 노트 1개를 추가 점검했다.
- 확률론, 확률과정, 통계적 추론, 확률 부등식, 기술통계, Newsvendor Model의 독립적인 HyeWiki 초안 페이지를 생성했다.
...
## [2026-08-11] revise | 계층형 상세 위키 구조와 개인화 제거

- 모든 지식 페이지에 `빠른 이해` 층을 추가하고, 원문을 다시 열지 않아도 작동 원리와 한계를 설명할 수 있도록 내용을 확장했다.
- 공통 이해 기준을 정의·경계·관계, 문제·목적·역할, 구성·메커니즘, 가정·조건, 예시, 한계·불확실성으로 정리했다.
- 개념·방법론·논문·웹 자료·주제·아이디어에 유형별 상세 섹션을 추가하고, 개인 연구 맥락과 개인화 관련성 섹션을 제거했다.
- `raw/` 원본은 수정하지 않았으며, 정책·템플릿·운영 기록만 새 위키 기준에 맞게 갱신했다.
- 제목을 먼저 보여 주도록 내부 링크와 `wiki/index.md`를 갱신했다.
- `raw/web/`의 원본 파일명과 frontmatter의 `raw_file` 경로는 원본 추적을 위해 유지했다.

## [2026-08-02] revise | 웹 자료 페이지 파일명 규칙 통일

- `wiki/web/`의 자료 요약도 제목 우선 `<자료 제목> (발행연도)` 형식으로 통일하고 Nature Editorial 페이지를 갱신했다.
- `raw/web/` 원본 파일명과 frontmatter의 `raw_file` 경로는 원본 추적을 위해 유지했다.
