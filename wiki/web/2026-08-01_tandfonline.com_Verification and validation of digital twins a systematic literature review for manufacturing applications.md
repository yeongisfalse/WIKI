...
이 systematic literature review(SLR)는 제조 분야 DT에서 verification·validation(V&V)이 실제로 어떻게 수행되는지, 그리고 두 개념이 구분되는지를 분석한다. 결론은 **둘 다 수행했다고 보고한 연구가 적고**, V&V의 표준 절차와 목적에 대한 합의도 부족하다는 것이다. 신뢰할 수 있는 DT를 만들려면 DT의 capability·context of use·요구사항과 연결된 V&V가 필요하다.
...
- Phase 2는 V&V 활동의 유형, 목적, 기법과 DT 성숙도·적용 분야의 관계를 분류했다.
...
- verification만 보고한 연구는 31%, validation만 보고한 연구는 35%였다.
- 둘 다 수행했다고 보고한 연구 중 47%는 V&V를 서로 다른 목적의 과정이 아니라 동의어처럼 사용했다.
- 자주 쓰인 근거는 case study, 물리·가상 출력 비교, KPI·오차 지표, 요구사항 추적, 모델 기반 시스템 엔지니어링 등이었다.

### 3. 논문이 제안하는 구분

- **Verification**: DT가 명세된 요구사항을 충족하고 구현이 개념 모델을 올바르게 반영하는지 증거를 제시하는 과정
- **Validation**: 특정 context of use에서 DT가 물리 시스템을 정확히 대표하고 의도된 운영 목적을 달성하는지 증거를 제시하는 과정

## 출처와 맥락

- 저자: Julia Bitencourt, Ana Wooley, Gregory Harris
- 저널: *International Journal of Production Research*, 63(1), 342–370 (2025)
- 연구는 ISO 23247, 4R framework, 모델링·시뮬레이션 V&V 관행을 연결해 DT의 신뢰성 문제를 다룬다.

## 신뢰도와 근거

- 평가: `high`
- 근거: Taylor & Francis 공식 본문에서 SLR 검색·선정 절차, 4R 분류, V&V 결과와 결론을 확인했다.
- 범위 주의: 포함 데이터베이스·영어 문헌·저자들의 분류 기준에 의존하므로 모든 DT 연구를 대표한다고 단정하지 않는다.

## 혜영님의 연구와의 관련성

- 최적화 결과를 실제 시스템에 적용하려면 “모델이 요구사항대로 구현되었는가(verification)”와 “사용 목적에 맞게 실제를 대표하는가(validation)”를 분리해 기록해야 한다.
...
- SLR의 V&V 분류는 상당 부분 각 논문의 보고와 저자들의 판정에 기반한다.
- 4R 비율은 선택된 157편에 대한 결과이며 전체 분야의 절대 비율이 아니다.
- V&V 표준화와 trust metric은 연구 방향으로 제안되었지만, 본 논문이 완성된 표준을 제공하는 것은 아니다.

## 연결 후보

- [[concepts/Model Verification and Validation]] — verification·validation의 목적과 증거
- [[concepts/Digital Twin Maturity]] — 4R framework와 capability 수준
- [[concepts/Digital Twin]] — simulation과 동기화된 DT의 구분
- [[methods/Simulation-based Optimization]] — 최적화 결과의 사용 맥락 검증
- [[topics/Trusted Digital Twin]] — 신뢰성·요구사항 추적·V&V 연구 지도
