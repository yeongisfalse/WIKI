...
- 이 주제는 스마트 제조의 디지털트윈을 제품 수명주기, 제조 계층, 디지털트윈 기능이라는 세 축으로 분해해 문헌과 사례를 배치하는 지도다.
- 같은 “디지털트윈”이라도 설계 검증, 공정 모니터링, what-if 분석, 최적화, 제어 중 무엇을 지원하는지에 따라 필요한 데이터·모델·검증 증거가 달라진다.
- 문서를 읽을 때는 대상·수명주기·기능의 조합을 먼저 찾고, 그 조합이 실제 시스템의 데이터 흐름과 의사결정으로 어떻게 구현되었는지 확인한다.
...
새 자료를 이 주제에 연결할 때는 먼저 적용 대상을 제품·기계·공정·공장·물류 중 하나 이상으로 표시한다. 다음으로 수명주기 단계와 기능을 지정하고, 시스템이 단순 표현인지 실시간 동기화인지, 예측·what-if·최적화·제어까지 수행하는지 구분한다.

비교할 때는 네 가지 질문을 사용한다.

1. 데이터는 어떤 물리 상태를 얼마나 자주 갱신하는가?
2. 디지털 모델은 설명·예측·시나리오 비교·최적화 중 어느 역할을 수행하는가?
3. 결과는 모니터링 화면에 머무는가, 아니면 승인·계획·제어로 전달되는가?
4. 목적을 충족했다는 근거와 모델 변경 시 재검토 절차가 있는가?

## 대표 사례와 차이

인간-로봇 협업 사례는 작업·자원·이벤트와 Digital Mirror를 연결해 현장 협업을 분석하는 데 초점을 둔다. MaaS 사례는 IoT platform과 Advanced Plant Model, FlexSim, OptQuest를 연결해 전략·운영 의사결정을 함께 다룬다. 두 사례는 모두 simulation-based DT이지만, 시스템 경계·의사결정 시간 규모·KPI·데이터 계약이 다르므로 동일한 성능 결과로 비교할 수 없다.

## 주제를 읽을 때의 확인 질문

- 사례가 주장하는 capability가 실제 데이터·모델·운영 증거로 뒷받침되는가?
- 여러 계층과 수명주기를 연결할 때 식별자·버전·상태 의미가 유지되는가?
- 최적화 결과의 실행 조건, 사람의 승인 지점, 실패 시 rollback이 정의되어 있는가?
