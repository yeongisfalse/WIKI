...
1. **상태 수집**: 센서·MES·IoT platform에서 현재 상태, 재공품, 수요, 결함과 자원 정보를 수집한다.
2. **상태 동기화**: Advanced Plant Model 또는 동등한 상태 모델에 데이터를 반영한다.
3. **예측·what-if**: 이산사건 시뮬레이션으로 후보 정책과 시나리오를 비교한다.
4. **최적화**: 생산순서, 작업 할당, 자원 수, 레이아웃 또는 공정 파라미터를 최적화한다.
5. **검토·실행**: 불확실성 범위와 제약조건을 확인한 뒤 사람이 승인하거나 실행 시스템에 계획을 전달한다.
6. **관찰·갱신**: 실제 결과와 예측 결과를 비교하고, 모델·데이터·목적함수를 갱신한다.
